...
//获取组件存放路径
Dates.getPath = "css/",  //修改为css下的laydate.css  ----zwt
//(function(){
...
Dates.use = function(lib, id){
    ...
    link.type = 'text/css';
    link.rel = 'stylesheet';
//    link.href = Dates.getPath + lib + as[5];  注掉路径  --zwt
    id && (link.id = id);
...
Dates.run = function(options){
    ...
    //chgSkin(options.skin || config.skin)  注掉 ,改用唯一样式  ---zwt
};
...
Dates.check = function(){
    ...
    if(value.replace(/\s/g, '') !== ''){
        if(!exp.test(value)){
            Dates.elem[as.elemv] = '';
            //Dates.msg('日期不符合格式，请重新选择。'); //注掉laydate自带提示框  -zwt
            Dates.close();                         //关闭日期框
            return 1;
        } else if(isvoid[0]){
            Dates.elem[as.elemv] = '';
...
Dates.viewDate = function(Y, M, D){
    ...
    if(config.format=="YYYY-MM"||D==undefined){//如果只选择年月  -zwt
    	De.setFullYear(Y, M);
    	log.ymd = [De.getFullYear(), De.getMonth()];
    	log.ymd1=[log.ymd[0],log.ymd[1]+1];
    	
    	De.setFullYear(log.ymd[0],log.ymd1[1]-1);    	
    	var a1=[De.getFullYear(), De.getMonth()+1];
    	De.setFullYear(log.ymd[0],log.ymd1[1]-2);
    	var a2=[De.getFullYear(), De.getMonth()+1];
    	De.setFullYear(log.ymd[0],log.ymd1[1]-3);
    	var a3=[De.getFullYear(), De.getMonth()+1];
    	De.setFullYear(log.ymd[0],log.ymd1[1]-4);
    	var a4=[De.getFullYear(), De.getMonth()+1];
    	De.setFullYear(log.ymd[0],log.ymd1[1]-5);
    	var a5=[De.getFullYear(), De.getMonth()+1];
    	$("#laydate_table").empty();
    	$(".laydate_body .laydate_box").css("color","#333");
    	var html="<div style='color: #67BFE4;font-size:1.3rem;'>快速选择</div>";
    	html+="<div class='zwt-click' data-zwt0="+a5[0]+" data-zwt1="+a5[1]+">"+a5[0]+"年"+a5[1]+"月"+"</div>";
    	html+="<div class='zwt-click' data-zwt0="+a4[0]+" data-zwt1="+a4[1]+">"+a4[0]+"年"+a4[1]+"月"+"</div>";
    	html+="<div class='zwt-click' data-zwt0="+a3[0]+" data-zwt1="+a3[1]+">"+a3[0]+"年"+a3[1]+"月"+"</div>";
    	html+="<div class='zwt-click' data-zwt0="+a2[0]+" data-zwt1="+a2[1]+">"+a2[0]+"年"+a2[1]+"月"+"</div>";
    	html+="<div class='zwt-click' style='background-color:#67BFE4;color:#fff;' data-zwt0="+a1[0]+" data-zwt1="+a1[1]+">"+a1[0]+"年"+a1[1]+"月"+"</div>";
    	$("#laydate_table").append(html);
    	/**
    	 * 新增快速选择菜单
    	 */
    	$(".zwt-click").click(function(){
    		var year=$(this).attr("data-zwt0");
    		var month=$(this).attr("data-zwt1");
    		$(this).css({"background-color":"#67BFE4","color":"#fff"});
    		log.ymd[year,month];
    		Dates.creation([year, month]);
    	});
    }else{
 	    De.setFullYear(Y, M, D);
	    log.ymd = [De.getFullYear(), De.getMonth(), De.getDate()];
	    $(".laydate_table").css("display","block");
	    $(".laydate_bottom").css("margin","0");
    }
...
Dates.initDate = function(format){
    ...
    if(ymd.length < 2){//放开对月份的处理  也就是  YYYY-MM  符合要求   ----zwt
...
Dates.orien = function(obj, pos){
	var a=Dates.options.divID;
	if(a){
		var tops, rect = document.getElementById(a).getBoundingClientRect();
 ...
 * 新增快速选择菜单
 */
function speedClick(year,month){
